...
//                Local local1 = view.geti
    ...
    public void GuardarComentario(View view) throws UnsupportedEncodingException {
        ...
        try {
            String url = "http://" + LoginActivity.dir_ip + "/MyEvents/rs/Comentarios/agregar-comentario-local?id_local=" + codigo + "&loc_descripcion=" + comentario + "&id_user=" + LoginActivity.cod_per;
    ...
    private void ListarComentarios() {
        clienteRest = new ClienteRest(this);
        try {
            String url = "http://" + LoginActivity.dir_ip + "/MyEvents/rs/Comentarios/list-comentarios-local?id_local=" + codigo;
            System.out.println("URL :  ==> " + url);
